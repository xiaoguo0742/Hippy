...
class NetworkInspector {
  ...
  void onResponseReceived(EngineContext context, String requestId, Response? httpResponse) {
    Map responseHeader = httpResponse?.headers.map.map((key, value) => MapEntry(key, value.join(';'))) ?? {};
    Map requestHeader = httpResponse?.requestOptions.headers.map((key, value) {
          if (value is List) {
  ...
  void onRequestWillBeSent(EngineContext context, String requestId, HttpRequest request) {
    Map<String, String> requestHeader = request.getHeaders().map((key, value) {
      if (value is List) {
        return MapEntry(key, value.join(';'));
      }
      return MapEntry(key, value.toString());
    ...
    Map entryMap = {kHttpRequestBytes: base64Encode(utf8.encode(postData))};
    Map requestMap = {
    ...
    int timeStamp = NetworkUtil.getTimeStamp();
    Map defaultInitiatorMap = {"type": "other"};
    Map jsonMap = {
