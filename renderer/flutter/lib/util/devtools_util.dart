...
class DevtoolsUtil {
  ...
  static void addFrameCallback(RenderViewModel viewModel, VoltronArray array, Promise? promise) {
    if (array.size() > 0) {
      var paramsMap = array.get<VoltronMap>(0);
      var callbackId = paramsMap?.get<int>(kFrameCallbackId);
  ...
  static Future<void> getScreenShot(RenderViewModel viewModel, VoltronArray array, Promise? promise) async {
    ...
    /// find rootKey
    GlobalKey? globalKey = viewModel.context.getInstance(viewModel.rootId)?.rootKey;
    ...
    String base64Content = pngByteList == null ? "" : base64Encode(pngByteList);

    /// promise callback
    var resultMap = VoltronMap();
    resultMap.push(kScreenShot, base64Content);
    var width = viewModel.width?.toInt();
    var height = viewModel.height?.toInt();
    resultMap.push(kScreenWidth, width);
    resultMap.push(kScreenHeight, height);
